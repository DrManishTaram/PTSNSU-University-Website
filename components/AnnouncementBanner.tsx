...
const AnnouncementBanner: React.FC = () => {
  ...
  return (
    <div className="bg-turmeric-50/80 backdrop-blur-sm border-b border-turmeric-100/50 text-earth-900 relative z-40 h-[38px] overflow-hidden flex items-center">
      <div className="container mx-auto px-2 lg:px-4 xl:max-w-[98%] flex items-center gap-4 h-full">
        
        {/* Label Icon - Stationary */}
        <div className="flex-shrink-0 flex items-center gap-3 z-10 bg-turmeric-50 pr-4 border-r border-turmeric-200/50 h-full">
            <span className="flex-shrink-0 bg-turmeric-100 p-1 rounded-full text-turmeric-600 animate-pulse">
                <Megaphone size={14} />
            </span>
            <span className="font-bold text-[11px] uppercase tracking-wider text-turmeric-800 hidden md:block">
                Announcements
            </span>
        </div>
        ...
        <div className="flex-1 overflow-hidden relative h-full flex items-center group">
            <div className="flex items-center space-x-12 whitespace-nowrap animate-marquee group-hover:[animation-play-state:paused] will-change-transform">
                ...
                {[...announcements, ...announcements, ...announcements].map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3 text-xs font-medium">
                        ...
                        <span className="text-earth-800">{item.text}</span>
            ...
            <div className="absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r from-turmeric-50 to-transparent z-10 pointer-events-none"></div>
            <div className="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-turmeric-50 to-transparent z-10 pointer-events-none"></div>
        </div>
      </div>
