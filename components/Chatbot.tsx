...
const Chatbot: React.FC = () => {
  ...
  return (
    <>
      {/* Floating Admission Button - Hidden when Contact Section is visible */}
      <a
        href="/admission-notification"
        className={`fixed bottom-24 right-6 z-40 transition-all duration-500 group ${isContactVisible ? 'opacity-0 translate-y-10 pointer-events-none' : 'opacity-100 translate-y-0'}`}
        style={{ animationDuration: '3s' }}
      >
        <div className="relative">
             <div className="bg-gradient-to-r from-violet-700 to-violet-600 text-white px-5 py-3 rounded-full shadow-[0_4px_20px_rgba(109,40,217,0.4)] border-2 border-white flex items-center gap-2 font-bold text-sm tracking-wide uppercase hover:scale-105 transition-transform">
                <span>Admission Open</span>
                <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
             </div>
             {/* Alert Badge */}
             <div className="absolute -top-3 -left-2 bg-turmeric-400 text-earth-900 text-[10px] font-black px-2 py-0.5 rounded shadow-sm border border-white transform -rotate-12 animate-pulse">
                NOW!
             </div>
        </div>
      </a>

      {/* Floating Toggle Button - Hidden when Contact Section is visible */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className={`fixed bottom-6 right-6 z-50 shadow-2xl transition-all duration-500 flex items-center justify-center gap-2.5
          ${isOpen 
      ...
      >
        ...
        {!isOpen && (
            <span className="font-bold text-[15px] tracking-wide whitespace-nowrap font-sans">Chat with us</span>
        )}
